      ...
      <div className="relative z-10 w-full max-w-md bg-slate-900 border-l border-slate-800 h-full flex flex-col text-slate-100 overflow-hidden pb-[env(safe-area-inset-bottom)]">
        ...
        <div className="p-4 border-b border-slate-800 flex items-center justify-between bg-slate-950">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h2 className="font-semibold text-sm text-slate-100">Guest Folio &amp; Tab</h2>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-3">
          {guestSession.rounds.length === 0 ? (
            <div className="h-48 flex flex-col items-center justify-center text-center text-slate-500">
              ...
              <p className="text-xs font-medium text-slate-400">No orders placed yet</p>
            ...
            guestSession.rounds.map((round) => (
              <div
                key={round.id}
                className="bg-slate-950 border border-slate-800 rounded-md p-3 space-y-2 text-xs"
              >
                {/* Round Header */}
                <div className="flex items-center justify-between pb-1.5 border-b border-slate-800 font-mono text-[11px]">
                ...
                <div className="space-y-1.5">
                  {round.items.map((item) => (
                    ...
                    >
                      ...
                      <span className={`font-mono text-xs ${item.isVoided ? 'text-slate-600 line-through' : 'text-slate-300'}`}>
        ...
        <div className="p-4 border-t border-slate-800 bg-slate-950 space-y-3">
          <div className="space-y-1 text-xs font-mono">
            ...
            <div className="flex justify-between font-bold text-slate-100 pt-1.5 border-t border-slate-800 text-sm">
              ...
              <span>₹{guestSession.totalAmount.toFixed(2)}</span>
            </div>
          </div>
